"""Script to manually label Cassini magnetometer data for neural network testing

TODO:
    * Re-configure for Cassini
...
def load_data():
    ...
    data = pd.read_csv('%s/CASSINI_2006_01_PROC.csv' % folder, names=data_names, dtype=float, header=0)\
...
def load_labels():
    ...
    labels = pd.read_csv('%s/VOY1_Labels.csv' % folder, names=header, dtype=str, header=0, sep=',', index_col='CLASS')
